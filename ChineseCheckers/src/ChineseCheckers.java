...
public class ChineseCheckers extends JFrame {
	...
	// Игру делали: Постников Василий, Сергей Гладких, Ямщиков Василий
    ...
    public class Board extends JPanel {
    	...
    	// Инициализирует все объекты ImageCon
	    ...
	    // Показывает параметры игры, которые изначально отображаются при запуске игры
	    ...
	    // Скрывает параметры игры, которые изначально отображаются при запуске игры
	    ...
	    // Создает элементы информации о повороте, которые будут отображаться
	    ...
	    // Создает объекты опций для выбора режима игры
	    ...
	    // устанавливает значки JLabels, соответствующие JLabel 'p'
	    ...
	    // Скрывает все выделенные параметры, доступные для JLabel p
	    ...
	    // Возвращает true только в том случае, если JLabel p пуст
	    ...
	    // рисует фоновое изображение на доске
	    @Override
	    ...
	    // Mouse listener для элементов доски
		class BoardMouseHandler implements MouseListener {
	        ...
	        // обрабатывает событие, когда мышь выходит из области
